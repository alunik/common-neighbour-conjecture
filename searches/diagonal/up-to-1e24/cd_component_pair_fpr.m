...
require assigned CD24SimpleId and assigned CD24K and
        assigned CD24ExpectedCases and assigned CD24ExpectedOrder and
        assigned CD24ExpectedDegree: "set all CD24 pair task parameters";
if Type(CD24SimpleId) eq MonStgElt then CD24SimpleId := StringToInteger(CD24SimpleId); end if;
if Type(CD24K) eq MonStgElt then CD24K := StringToInteger(CD24K); end if;
if Type(CD24ExpectedCases) eq MonStgElt then CD24ExpectedCases := StringToInteger(CD24ExpectedCases); end if;
if Type(CD24ExpectedOrder) eq MonStgElt then CD24ExpectedOrder := StringToInteger(CD24ExpectedOrder); end if;
if Type(CD24ExpectedDegree) eq MonStgElt then CD24ExpectedDegree := StringToInteger(CD24ExpectedDegree); end if;
SetColumns(0);

tuple, t_order := SimpleGroupId(CD24SimpleId);
require t_order eq CD24ExpectedOrder: "simple order mismatch";
...
degree := t_order^(CD24K - 1);
require degree eq CD24ExpectedDegree: "component degree mismatch";
...
require #candidates eq CD24ExpectedCases: "component quotient census mismatch";
...
LiftOuterTop := function(b)
    ...
    a := q @@ qmap;
    sigma := b @ dprojs[2];
    diagonal_a := &*[W | a @ bins[i] : i in [1 .. CD24K]];
    return diagonal_a * (sigma @ topin);
end function;

label := Sprintf("CD24CP_sid%o_k%o", CD24SimpleId, CD24K);
print "CD24_COMPONENT_PAIR_FPR_V1";
printf "CD24_COMPONENT_PAIR|%o|sid=%o|T=%o|T_order=%o|k=%o|degree=%o|cases=%o|Out=%o\n",
       label, CD24SimpleId, SimpleGroupName(CD24SimpleId), t_order,
       CD24K, degree, #candidates, Order(Q);
...
for case_number in [1 .. #candidates] do
    ...
    independent_t := [t @ bins[i] : i in [1 .. CD24K], t in Generators(T)];
    diagonal_t := [&*[W | t @ bins[i] : i in [1 .. CD24K]] : t in Generators(T)];
    G := sub<W | independent_t cat b_lifts>;
    H := sub<W | diagonal_t cat b_lifts>;
    require Order(G) eq t_order^CD24K * Order(B): "bad component G order";
    require Order(H) eq t_order * Order(B): "bad component H order";

    printf "CD24_COMPONENT_ACTION|%o|case=%o|B=%o|top=%o|outer_image=%o|top_kernel=%o|G=%o|H=%o\n",
    ...
    printf "CD24_COMPONENT_CASE_COMPLETE|%o|case=%o|prime_H_classes=%o|prime_G_classes=%o|bound_num=%o|bound_den=%o|lt_half=%o\n",
           label, case_number, #prime_classes, #intersections, Numerator(bound),
           Denominator(bound), bound lt 1/2;
end for;
printf "CD24_COMPONENT_PAIR_COMPLETE|%o|cases=%o\n", label, #candidates;
quit;
