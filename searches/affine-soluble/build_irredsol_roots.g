#############################################################################
## List the guardian roots needed to cover every IRREDSOL action with
## 8192 <= p^n <= 2^24-1 and n >= 2.
##
## Output columns:
## root_index, degree, n, p, splitting_degree, guardian
#############################################################################
...
Print("root_index\tdegree\tn\tp\td\tguardian\n");
ASX_RootIndex := 0;
        ...
        if Length(ASX_Indices) > 0 then
            ...
            if ASX_m = 1 then
                ...
                ASX_Guardians := Set(List(ASX_Indices,
                    k -> IRREDSOL_DATA.GROUPS[ASX_m][ASX_q][k][1]));
            fi;
