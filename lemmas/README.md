...
| [`twisted_wreath_census.g`](twisted_wreath_census.g) | Lemma 5.1 | a table-of-marks census giving the exact number `64,790,243` of regular suborbits of `A5 twr S6` (and the companion `A6`-top census) |
...
Each file prints the checks it performs, stops with an error if any check
fails, and finishes in a few seconds. The census uses the TomLib package,
which is distributed with GAP. The other files need only core GAP.
